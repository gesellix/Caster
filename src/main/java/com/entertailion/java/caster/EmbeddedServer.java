...
/**
 * Custom HTTP server to send video file to ChromeCast device media player; support range requests
 *
...
public class EmbeddedServer extends HttpServer {
  ...
  public Response serve(String uri, String method, Properties header, Properties parms, Properties files) {
    try {
       ...
       * TODO if ("POST".equalsIgnoreCase(method)){ Log.d(LOG_TAG,
			 * "A POST is received"); Log.d(LOG_TAG, "action Param is:" +
			 * parms.getProperty("action")); return new Response("200",
			 * "text/plain", "Ok"); } else if ("GET".equalsIgnoreCase(method)){
			 * String action = parms.getProperty("action"); return new
   ...
   */
  public Response serveFile(String uri, Properties header, File homeDir, boolean allowDirectoryListing, Properties parms) {
    Response res = null;
    ...
    try {
      if (res == null) {
        ...
        File file = new File(systemProperties.getProperty(EmbeddedServer.CURRENT_FILE));
